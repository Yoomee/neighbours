class HomeController < ApplicationController
  ...
  def index
    ...
    if current_user.try(:has_lat_lng?)
      ...
      @general_offers_json = GeneralOffer.where('users.is_deleted IS FALSE').from_live_neighbourhood.visible_to_user(current_user).to_json(:current_user => current_user, :only => [:id], :methods => [:lat, :lng, :miles_from_s, :title, :user_first_name])
    else
      @needs_json ||= []
      @users_json ||= []
    end
  ...
  def get_items_for_cycle_columns
    ...
    if current_user.try(:has_lat_lng?)
      @general_offers = GeneralOffer.joins(:user).where('users.is_deleted IS FALSE').from_live_neighbourhood.visible_from_location(current_user.lat, current_user.lng, :order_by_closest => true, :limit => 20).at_least(20).select{|need| need.removed_at.nil?}
      ...
      @general_offers = GeneralOffer.where('users.is_deleted IS FALSE').from_live_neighbourhood.visible_to_user(current_user).order('created_at DESC').limit(20).at_least(20).select{|need| need.removed_at.nil?}
