class HomeController < ApplicationController
  ...
  def get_items_for_cycle_columns
    ...
    if current_user.try(:has_lat_lng?)
      @general_offers = GeneralOffer.from_live_neighbourhood.visible_from_location(current_user.lat, current_user.lng, :order_by_closest => true, :limit => 20).where(:removed_at => nil).at_least(20)
      ...
      @general_offers = GeneralOffer.from_live_neighbourhood.visible_to_user(current_user).order('created_at DESC').where(:removed_at => nil).limit(20).at_least(20)
      @helped = Need.from_live_neighbourhood.resolved.order('created_at DESC').limit(20).at_least(20).where(:removed_at => nil) if @general_offers.empty?
